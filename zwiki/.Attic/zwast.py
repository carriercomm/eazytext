...
BT_HEADCELL      = 'BT_HEADCELL'
BT_HEADCELLSTYLE = 'BT_HEADCELLSTYLE'
BT_CELL          = 'BT_CELL'
BT_CELLSTYLE     = 'BT_CELLSTYLE'
...
class BigTable( Node ) :
    """class to handle `bigtable` grammar."""
    def __init__( self, parser, btopen, opennl, rows,
                  btclose=None, closenl=None, skip=False ) :
        self.parser  = parser
        self.btopen  = btopen
        self.opennl  = Newline( parser, opennl )
        self.rows    = rows
        self.btclose = btclose
        self.closenl = Newline( parser, closenl )
        self.skip    = skip
        try :
            self.table_props = eval( btopen.strip(' \t')[2:] )
        except :
            self.table_props = {}
        self.caption     = self.table_props.get( 'caption', '' )
        self.cellspacing = self.table_props.get( 'cellspacing', '0' )
        self.cellpadding = self.table_props.get( 'cellpadding', '5' )
        self.border      = self.table_props.get( 'border', '1px' )
    ...
    def dump( self ) :
        text = self.btopen + self.opennl.dump() + self.rows.dump()
        if not self.skip :
            text += self.btclose + self.closenl.dump()
        return dump
    
    def show( self, buf=sys.stdout, offset=0, attrnames=False,
              showcoord=False ) :
        lead = ' ' * offset
        buf.write( lead + 'bigtable :' )
        if showcoord :
            buf.write( ' (at %s)' % self.coord )
        buf.write( '\n' )
        self.rows.show( buf, offset + 2, attrnames, showcoord )


class BigtableRows( Node ) :
    """class to handle `bigtablerows` grammar"""
...
class BigtableRow( Node ) :
    """class to handler `bigtablerow` grammar"""
    def __init__( self, parser, rowmarkup, nl=None, cells=None, skip=False ) :
        self.parser    = parser
        self.rowmarkup = rowmarkup
        self.skip      = skip
        if not self.skip :
            self.newline   = Newline( parser, nl )
            self.cells     = cells
            try :
                self.trow_props = eval( rowmarkup.strip(' \t')[2:] )
            except :
                self.trow_props = {}
    ...
    def dump( self ) :
        text = self.rowmarkup
        if not self.skip :
            text += self.rowmarkup + self.newline.dump() + self.cells.dump()
        return text

    def show( self, buf=sys.stdout, offset=0, attrnames=False,
              showcoord=False ) :
        lead = ' ' * offset
        buf.write( lead + 'bigtablerow :' )
        if showcoord :
            buf.write( ' (at %s)' % self.coord )
        buf.write( '\n' )
        self.cells.show( buf, offset + 2, attrnames, showcoord )


class BigtableCells( Node ) :
    """class to handle `bigtablecells` grammar"""
    def __init__( self, parser, cellmarkup, cell, nl, type=None ) :
        """`cell` can be `Empty` or `TextContents` object"""
        self.parser = parser
        cellprops = {}
        if type == BT_HEADCELLSTYLE :
            try :
                cellprops = eval( cellmarkup.strip( ' \t' )[7:-1] )
            except :
                cellprops = {}
        elif type == BT_CELLSTYLE :
            try :
                cellprops = eval( cellmarkup.strip( ' \t' )[6:-1] )
            except :
                cellprops = {}
        self.cells  = [ [ cellprops, cellmarkup, cell, 
                          Newline( parser, nl ), type 
                        ] ]

    def appendcell( self, cellmarkup, cell, nl, type=None ) :
        cellprops = {}
        if type == BT_HEADCELLSTYLE :
            try :
                cellprops = eval( cellmarkup.strip( ' \t' )[7:-1] )
            ...
            try :
                ...
                cellprops = {}
        self.cells.append([ cellprops, cellmarkup, cell, 
                            Newline( self.parser, nl ), type ])
    ...
    def dump( self ) :
        text = ''
        for css, cmrkup, cell, nl, type in self.cells :
            text += cmrkup + cell.dump() + nl.dump()
        return text

    def show( self, buf=sys.stdout, offset=0, attrnames=False,
              showcoord=False ) :
        lead = ' ' * offset
        buf.write( lead + 'bigtablecells :' )
