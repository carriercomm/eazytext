...
#       usernames, projectnames, tags, WikiPages
#   * Macros for,
#       - project description
#       - project components
#       - project milestones
...
#       - mailing lists
#       - featured wiki, featured tickets, featured reviews
#       - downloads
#
#       - Recent N projects, 
#       - Recent N users
...
class ZWParser( object ):
    ...
    def p_bigtable( self, p ) :                         # BigTable
        """bigtable      : BIGTABLE_OPEN NEWLINE bigtablerows BIGTABLE_CLOSE NEWLINE
                         | BIGTABLE_OPEN NEWLINE bigtablerows ENDMARKER"""
        if len(p) == 6 :
            p[0] = BigTable( p.parser, p[1], p[2], p[3], p[4], p[5] )
        elif len(p) == 5 :
            p[0] = BigTable( p.parser, p[1], p[2], p[3], p[4], skip=True )
        else :
            raise ParseError( "unexpected rule-match for bigtable")

    def p_bigtablerows( self, p ) :                     # BigtableRows
        """bigtablerows  : bigtablerow
                         | bigtablerows bigtablerow"""
        if len(p) == 2 and isinstance( p[1], BigtableRow ) :
            p[0] = BigtableRows( p.parser, p[1] )
        elif len(p) == 3 and isinstance( p[1], BigtableRows ) \
             and isinstance( p[2], BigtableRow ) :
            p[1].appendrow( p[2] )
            p[0] = p[1]
        else :
            raise ParseError( "unexpected rule-match for bigtablerows")

    def p_bigtablerow( self, p ) :                      # BigtableRow
        """bigtablerow   : BIGTABLE_ROW NEWLINE bigtablecells
                         | NEWLINE
                         | BIGTABLE_BREAK"""
        if len(p) == 4 and isinstance( p[3], BigtableCells ) :
            p[0] = BigtableRow( p.parser, p[1], p[2], p[3] )
        elif len(p) == 2 :
            p[0] = BigtableRow( p.parser, p[1], skip=True )
        else :
            raise ParseError( "unexpected rule-match for bigtablerow")

    def p_bigtablecells_1( self, p ) :
        """bigtablecells : BIGTABLE_HEADCELL text_contents NEWLINE
                         | BIGTABLE_HEADCELL empty NEWLINE
                         | bigtablecells BIGTABLE_HEADCELL text_contents NEWLINE
                         | bigtablecells BIGTABLE_HEADCELL empty NEWLINE"""
        if len(p) == 4 :
            p[0] = BigtableCells( p.parser, p[1], p[2], p[3], type=BT_HEADCELL )
        elif len(p) == 5 :
            p[1].appendcell( p[2], p[3], p[4], type=BT_HEADCELL )
            p[0] = p[1]
        else :
            raise ParseError( "unexpected rule-match for bigtablecells_1")

    def p_bigtablecells_2( self, p ) :
        """bigtablecells : BIGTABLE_HEADCELLSTYLE text_contents NEWLINE
                         | BIGTABLE_HEADCELLSTYLE empty NEWLINE
                         | bigtablecells BIGTABLE_HEADCELLSTYLE text_contents NEWLINE
                         | bigtablecells BIGTABLE_HEADCELLSTYLE empty NEWLINE"""
        if len(p) == 4 :
            p[0] = BigtableCells( p.parser, p[1], p[2], p[3], type=BT_HEADCELLSTYLE )
        elif len(p) == 5 :
            p[1].appendcell( p[2], p[3], p[4], type=BT_HEADCELLSTYLE )
            p[0] = p[1]
        else :
            raise ParseError( "unexpected rule-match for bigtablecells_2")

    def p_bigtablecells_3( self, p ) :
        """bigtablecells : BIGTABLE_CELL text_contents NEWLINE
                         | BIGTABLE_CELL empty NEWLINE
                         | bigtablecells BIGTABLE_CELL text_contents NEWLINE
                         | bigtablecells BIGTABLE_CELL empty NEWLINE """
        if len(p) == 4 :
            p[0] = BigtableCells( p.parser, p[1], p[2], p[3], type=BT_CELL )
        elif len(p) == 5 :
            p[1].appendcell( p[2], p[3], p[4], type=BT_CELL )
            p[0] = p[1]
        else :
            raise ParseError( "unexpected rule-match for bigtablecells_3")

    def p_bigtablecells_4( self, p ) :
        """bigtablecells : BIGTABLE_CELLSTYLE text_contents NEWLINE
                         | BIGTABLE_CELLSTYLE empty NEWLINE
                         | bigtablecells BIGTABLE_CELLSTYLE text_contents NEWLINE
                         | bigtablecells BIGTABLE_CELLSTYLE empty NEWLINE"""
        if len(p) == 4 :
            p[0] = BigtableCells( p.parser, p[1], p[2], p[3], type=BT_CELLSTYLE )
        elif len(p) == 5 :
            p[1].appendcell( p[2], p[3], p[4], type=BT_CELLSTYLE )
            p[0] = p[1]
        else :
            raise ParseError( "unexpected rule-match for bigtablecells_4")
