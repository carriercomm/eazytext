...
def parse_zetalink( app, name, interzeta='' ) :
    """Parse 'name' into zeta understandable notation and convert them into
    relative url"""
    vals   = name.split( '.' )
    kwargs = {}
    ...
    return url and ( interzeta.rstrip('/') + '/' + url.lstrip('/') ) or ''

def parse_link( parser, href ) :
    """Parse href into ( interzeta, zetalink )"""
    href = href.strip( ' \t' )
    if href[0] == '@' :
        X     = href[1:].split( '%' )
        host  = parse_interzeta( parser.zwparser.app, X[0] )
        href1 = host and parse_zetalink( parser.zwparser.app, X[1], host ) or ''
    elif href[0] == '%' :
        href1 = parse_zetalink( parser.zwparser.app, href[1:] )
    return href1
